...
class AnalyticsTests(DatabaseTestCase, unittest.TestCase):
    def test_all_analytics_reports(self):
        ...
        PaymentService.create_payment(order.order_id, "TPBANK", "THANH TOÁN FULL", "ANALYTICS")
        ...
        self.assertTrue(AnalyticsService.get_revenue_report(datetime(2020, 1, 1), datetime(2030, 1, 1)))
        self.assertTrue(AnalyticsService.get_order_statistics())
        self.assertTrue(AnalyticsService.get_best_selling_products(5))
        self.assertEqual(AnalyticsService.get_low_stock_products(5)[0]["status"], "SẮP HẾT HÀNG")
...
class NotificationTests(DatabaseTestCase, unittest.TestCase):
    def test_notification_send_and_read(self):
        ...
        with patch("modules.notifications.services.send_email") as mocked_send:
            notification = NotificationService.send_preorder_notification(self.seed["preorder"].preorder_id, " Update ", " Production ")
        self.assertIsNotNone(notification)
        mocked_send.assert_called_once()
        self.assertEqual(UserNotification.query.count(), 1)
        self.assertEqual(NotificationService.get_user_notifications(self.seed["customer"].user_id)[0], notification)

    def test_notification_no_customer_and_validation(self):
        self.assertIsNone(NotificationService.send_preorder_notification(self.seed["preorder"].preorder_id, "Title", "Body"))
        with self.assertRaises(ValueError):
            NotificationService.send_preorder_notification(999, "T", "B")
        with self.assertRaises(ValueError):
            NotificationService.send_preorder_notification(self.seed["preorder"].preorder_id, "", "B")
        with self.assertRaises(ValueError):
    ...
    def test_email_failure_does_not_rollback_notification(self):
        ...
        with patch("modules.notifications.services.send_email", side_effect=RuntimeError("SMTP down")):
            notification = NotificationService.send_preorder_notification(self.seed["preorder"].preorder_id, "Title", "Body")
        self.assertIsNotNone(notification)
...
class HelperTests(DatabaseTestCase, unittest.TestCase):
    def test_all_serializers(self):
        ...
        self.assertEqual(preorder_helpers.serialize_preorder(self.seed["preorder"])["product"]["product_name"], product.product_name)

        order = OrderService.create_order(
            self.seed["customer"].user_id,
            [{"product_id": self.seed["stock_product"].product_id, "preorder_id": None, "quantity": 1}],
        )
        payment = PaymentService.create_payment(order.order_id, "TPBANK", "THANH TOÁN FULL", "HELPER")
        self.assertEqual(order_helpers.serialize_order(order)["order_id"], order.order_id)
        self.assertEqual(order_helpers.serialize_order_summary(order)["username"], "customer")
        self.assertEqual(order_helpers.serialize_order_item(order.order_items[0])["product_name"], "Áo có sẵn")
        self.assertEqual(order_helpers.serialize_payment(payment)["transaction_id"], "HELPER")
        ...
        self.assertEqual(inventory_helpers.serialize_inventory_transaction(tx)["transaction_type"], "XUẤT")
        ...
        if cart is None:
            ...
            CartService.add_item(self.seed["customer"].user_id, self.seed["stock_product"].product_id, 1)
        self.assertEqual(serialize_cart(cart)["total_items"], 1)
        self.assertEqual(serialize_cart_item(cart.items[0])["product_name"], "Áo có sẵn")

        wallet = self.seed["customer"].wallet
        self.assertEqual(wallet_helpers.serialize_wallet(wallet)["user_id"], self.seed["customer"].user_id)
        wallet_tx = __import__("modules.wallets.services", fromlist=["WalletService"]).WalletService.create_deposit_request(self.seed["customer"].user_id, 1000)
        self.assertEqual(wallet_helpers.serialize_wallet_transaction(wallet_tx)["transaction_type"], "NẠP TIỀN")

        notification = SimpleNamespace(notification_id=1, preorder_id=None, title="T", message="M", created_at=datetime.now(timezone.utc))
        self.assertEqual(notification_helpers.serialize_notification(notification)["title"], "T")

    def test_analytics_serializers_and_response_helpers(self):
        self.assertEqual(analytics_helpers.serialize_dashboard_summary({"total_revenue": 1, "total_orders": 2, "total_customers": 3, "total_products": 4, "low_stock_count": 5})["total_orders"], 2)
        self.assertEqual(analytics_helpers.serialize_revenue_report({"date": date.today(), "paid": 10, "refunded": 2, "revenue": 8})["revenue"], 8.0)
        self.assertEqual(analytics_helpers.serialize_order_statistic({"order_status": "X", "total": 1})["total"], 1)
        self.assertEqual(analytics_helpers.serialize_best_selling_product({"product_id": 1, "product_name": "P", "quantity_sold": 2})["quantity_sold"], 2)
        self.assertEqual(analytics_helpers.serialize_low_stock_product({"product_id": 1, "product_name": "P", "quantity": 0, "status": "HẾT HÀNG"})["quantity"], 0)
        self.assertEqual(analytics_helpers.serialize_customer_statistics({"total_customers": 1, "top_customers": [{"user_id": 1, "username": "u", "total_orders": 1, "total_paid": 2, "total_refunded": 1, "total_spent": 1}]})["top_customers"][0]["total_spent"], 1.0)
    ...
    def test_model_strings_and_inventory_statuses(self):
        ...
        self.assertIn(str(self.seed["preorder"].preorder_id), str(self.seed["preorder"]))
        inventory = self.seed["stock_product"].inventories[0]
