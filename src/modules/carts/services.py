...
class CartService:
    ...
    def add_item(
        ...
        if product.status == "IN_STOCK":
            ...
            preorder = db.session.scalar(stmt)

            if not preorder:
                raise ValueError("Sản phẩm hiện không có đợt preorder")

            today = date.today()

            if today < preorder.start_date or today > preorder.end_date:
                raise ValueError("Đợt preorder hiện không mở")
        ...
        stmt = select(CartItem).where(
            ...
            CartItem.preorder_id == preorder_id,
        )

        existing_item = db.session.scalar(stmt)
    ...
    def update_quantity(
        ...
        if product.status == "IN_STOCK":
            inventory = db.session.scalar(
                select(Inventory).where(
                    Inventory.product_id == item.product_id, Inventory.active.is_(True)
                )
            )
            ...
            if not preorder or not preorder.active:
                raise ValueError("Đợt preorder đã đóng")
