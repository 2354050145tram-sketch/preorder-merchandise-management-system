from sqlalchemy import Integer, Column, String, Boolean, DateTime, Enum, ForeignKey, DECIMAL
...
class WalletTransaction(db.Model):
    ...
    transaction_type = Column(Enum("NẠP TIỀN","THANH TOÁN","RÚT TIỀN","HOÀN TIỀN"), nullable=False)
    amount = Column(DECIMAL(15, 2), nullable=False)
    balance_before = Column(DECIMAL(15, 2), nullable=False)
    balance_after = Column(DECIMAL(15, 2), nullable=False)
    transaction_status = Column(Enum("CHỜ XỬ LÝ", "THÀNH CÔNG", "THẤT BẠI", "ĐÃ HỦY"), nullable=False)
